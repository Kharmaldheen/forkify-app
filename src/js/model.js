...
import { API_URL, API_KEY1, API_KEY2 } from './config.js';
...
export const loadRecipe = async function (id) {
  try {
    const data = await AJAX(`${API_URL}${id}?key=${API_KEY1}`);
    ...
    const data2 = await getCalories(
      `https://api.spoonacular.com/recipes/analyze?apiKey=${API_KEY2}&includeNutrition=true&includeTaste=true`,
      inputStrings
    );
    console.log(data2);
...
export const loadSearchResults = async function (query) {
  try {
    ...
    const data = await AJAX(`${API_URL}?search=${query}&key=${API_KEY1}`);
    console.log(data);
...
export const uploadRecipe = async function (newRecipe) {
  try {
    ...
    const data = await AJAX(`${API_URL}?key=${API_KEY1}`, recipe);
    console.log(data);
